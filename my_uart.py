...
class My_Uart485:
    ...
    def __init__(self,ser:serial.Serial ,DE_pin=None,RE_pin=None,timeout_s=10,timeout_callback=None,finish_callback=None):
        ...
        #cau hinh  de_pin  re_pin cho modbus
    ...
    def wait_3_5_char(self):
        time.sleep(self.time_3_5_char_s)

class My_Uart:
    
    def __init__(self,ser:serial.Serial, **kwargs):
        ...
        '''
            ...
            - rx loop
            - timer_rx
            - timer_detect_uart_stuck
        '''
        self.ser=ser
       
        self.is_wait_response=False

        self.timeout_callback= kwargs.get('timeout_callback',None)
        self.finish_callback= kwargs.get('finish_callback',None)
        
        self.rx_timer_interval=kwargs.get('rx_timer_interval',self._get_char_time()*10)
        self.timeout_timer_interval=kwargs.get('timeout_timer_interval',10)
    ...
    def _restart_rx_timer(self):
        if self.rx_timer:
            self.rx_timer.cancel()
        self.rx_timer=threading.Timer(self.rx_timer_interval,self._on_rx_timer_ellapse)
        self.rx_timer.start()

    def _restart_timeout_timer(self):
        if self.timeout_timer:
            self.timeout_timer.cancel()
        self.timeout_timer=threading.Timer(self.timeout_timer_interval,self._on_timeout_timer_ellapse)
        ...
        while not self._stop_event.is_set():
            try:
                if self.ser.in_waiting:
                    ...
                    print('rx_loop: wait lock')
                    with self._lock:
                        print('rx_loop: enter lock')
                        if self.is_wait_response==True:
                            self._cancel_timeout_timer()
                            self.is_wait_response=False
                
                        self.buff_rx.extend(data)   
                        self._restart_rx_timer()  
                time.sleep(0.001)
                
            except Exception as e:
                print(f'[UART_ERROR] {e}')
                time.sleep(0.1)
        
    def _on_rx_timer_ellapse(self):
        print('rx_timer:wait lock')
        with self._lock:
            print('rx_timer: enter lock')
            
            print('nhan du lieu day du')
            if self.buff_rx and self.finish_callback:
                self.finish_callback(self)
            self.buff_rx.clear()

    def _on_timeout_timer_ellapse(self):
        self.is_wait_response=False
        print('timeot_timer:wait lock')
        with self._lock:
            print('timeout_timer: enter lock')

            print('timeot wait response')
            if self.buff_tx and self.timeout_callback:
                self.timeout_callback(self)
            self.buff_tx.clear()
   
    def send_uart(self,data,is_wait=False):
        print('send_uart:wait lock')
        with self._lock:
            print('send_uart: enter lock')
   ...
   ##############################################
    ...
    def _get_char_time(self):
        return 10/self.ser.baudrate
